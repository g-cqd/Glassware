...
//  GlassToolbar
//
//  Previews demonstrating ToolbarContentBuilder patterns and spacer-based grouping.
//
    ...
    SampleContentView("Spacer Grouping", color: .blue.opacity(0.05))
      .glassToolbar(
        content: {
          ...
          .buttonStyle(.toolbarItem(isSelected: selectedTab == .profile))
        }
      )
...
private struct LayoutDistributionPreview: View {
  @State private var selectedTab: PreviewTab = .home
  @State private var distribution: ToolbarLayoutDistribution = .distributed

  var body: some View {
    VStack {
      Picker("Distribution", selection: $distribution) {
        Text("Natural").tag(ToolbarLayoutDistribution.natural)
        Text("Distributed").tag(ToolbarLayoutDistribution.distributed)
        Text("Centered").tag(ToolbarLayoutDistribution.centered)
      }
    ...
    .toolbarDistribution(distribution)
  }
}
...
private struct DistributionComparisonPreview: View {
  ...
  @ViewBuilder
  private func distributionRow(distribution: ToolbarLayoutDistribution, title: String) -> some View {
