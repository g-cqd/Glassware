...
/// // Tab with icon only
/// Button("Home", systemImage: "house") { }
///     .buttonStyle(.toolbarItem(style: .iconOnly(), isSelected: selectedTab == 0))
///
/// // Action button
...
/// ```
public struct ToolbarItemButtonStyle: ButtonStyle {
    // MARK: - Configuration

    /// The semantic intent of the button (tab or action).
    public let intent: ToolbarItemIntent

    /// The visual layout style (titleAndIcon, titleOnly, iconOnly).
    public let visualStyle: ToolbarItemVisualStyle
    ...
    private var selectionAnimation: Animation? {
        reduceMotion ? nil : .snappy(duration: ToolbarTokens.Animation.selectionDuration)
    }

    // MARK: - Initializer

    public init(
        intent: ToolbarItemIntent = .tab,
    ...
    private func labelContent(configuration: Configuration) -> some View {
        ...
        case .titleAndIcon(let image, let systemImage):
            if let systemImage {
                ...
                PrimaryToolbarLabelStyle.IconLabel(
                ...
                configuration.label
                    .labelStyle(PrimaryToolbarLabelStyle(isSelected: isSelected && intent == .tab))
            }
    ...
    private func foregroundColor(isPressed: Bool) -> Color {
        guard isEnabled else {
            return .primary.opacity(ToolbarTokens.Opacity.disabled)
        }
...
extension ButtonStyle where Self == ToolbarItemButtonStyle {
    /// Unified toolbar item style with configurable intent and visual style.
    ///
    /// - Parameters:
    ...
    public static func toolbarItem(
        ...
        ToolbarItemButtonStyle(intent: intent, style: style, isSelected: isSelected)
    }
}
