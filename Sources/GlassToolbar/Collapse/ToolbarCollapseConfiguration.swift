//
//  ToolbarCollapseConfiguration.swift
//  GlassToolbar
//
...
public enum ToolbarMergeSide: Int, Sendable, Equatable {
...
public enum ToolbarCollapseState: Sendable, Equatable {
...
///     configuration: ToolbarCollapseConfiguration(
///         mergeSide: .trailing,
...
public struct ToolbarCollapseConfiguration: Sendable, Equatable {
    /// Which side to merge primary content into.
    public let mergeSide: ToolbarMergeSide
    ...
    public init(
        mergeSide: ToolbarMergeSide = .trailing,
    ...
    public static let `default` = ToolbarCollapseConfiguration()

    /// Configuration merging to leading side.
    public static let leadingMerge = ToolbarCollapseConfiguration(mergeSide: .leading)

    /// Aggressive collapse with lower thresholds for quick response.
    public static let aggressive = ToolbarCollapseConfiguration(
        collapseThreshold: 40,
        expandThreshold: 15
    )

    /// Relaxed collapse with higher thresholds for less frequent transitions.
    public static let relaxed = ToolbarCollapseConfiguration(
